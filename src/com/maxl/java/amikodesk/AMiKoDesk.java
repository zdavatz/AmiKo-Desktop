...
public class AMiKoDesk {
	...
	// 0: Präparat, 1: Inhaber, 2: ATC Code, 3: Reg. Nr., 4: Wirkstoff, 5:
	// Therapie
	// -> {0, 1, 2, 3, 4, 5, 6};
	...
	static class WebPanel2 extends JPanel {
		...
		public WebPanel2() {
			...
			webBrowserPanel.setBorder(BorderFactory
					.createTitledBorder(titledBorder));
							...
							if (row_key.equals("0.0")) {
								...
								SwingUtilities.invokeLater(new Runnable() {
									...
									public void run() {
										...
										for (File file : files) {
											if (Utilities.appLanguage().equals("de")) {
												if (file.isFile() && file.getName().startsWith("warenkorb")
														&& file.getName().endsWith(".ser")) {
													String f = file.getName();
													list_of_carts.add(f.substring(0, f.lastIndexOf(".")));
												}
											} else if (Utilities.appLanguage().equals("fr")) {
												if (file.isFile() && file.getName().startsWith("panier")
														&& file.getName().endsWith(".ser")) {
													String f = file.getName();
													list_of_carts.add(f.substring(0, f.lastIndexOf(".")));
												}
		...
		public void saveShoppingCartWithIndex(final int n) {
			...
			if (file != null) {
				String filename = file.getAbsolutePath();
				// System.out.println("Saved shopping cart " + n + " to " +
				// filename);
				byte[] serialized_bytes = FileOps.serialize(m_shopping_basket);
				if (serialized_bytes != null) {
					FileOps.writeBytesToFile(filename, serialized_bytes);
			...
			if (med_search.size() < BigCellNumber) {
				for (int i = 0; i < med_search.size(); ++i) {
					...
					m.add("<html><b>" + ms.getTitle() 
							+ "</b><br><font color=gray size=-1>" + ms.getAuth() + "</font></html>");
					med_id.add(ms.getId());
				}
			} else {
				for (int i = 0; i < med_search.size(); ++i) {
		...
		if (med_search.size() < BigCellNumber) {
			for (int i = 0; i < med_search.size(); ++i) {
				...
				m.add("<html><b>" + ms.getSubstances()
						+ "</b><br><font color=gray size=-1>" + ms.getTitle()
						+ "</font></html>");
				med_id.add(ms.getId());
			}
		} else {
			for (int i = 0; i < med_search.size(); ++i) {
				Medication ms = med_search.get(i);
				m.add("<html><body style='width: 1024px;'><b>"
						+ ms.getSubstances()
						+ "</b><br><font color=gray size=-1>" + ms.getTitle()
						+ "</font></html>");
				med_id.add(ms.getId());
